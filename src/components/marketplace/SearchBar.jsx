...
export default function SearchBar({ value, onChange, location, onLocationChange }) {
  ...
  const selectedLocation = location || "All Ghana";
    ...
    <div className="flex w-full rounded-xl overflow-hidden shadow-sm border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700">
      ...
      <div className="relative" ref={dropdownRef}>
        ...
        >
          ...
          <span className="font-medium truncate max-w-[80px] sm:max-w-none">{selectedLocation}</span>
          <ChevronDown className="h-3.5 w-3.5 text-slate-400 shrink-0" />
        </button>
        {showDropdown && (
          <div className="absolute top-full left-0 mt-1 w-44 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-xl shadow-lg z-50 py-1 max-h-64 overflow-y-auto">
            {LOCATIONS.map((loc) => (
              <button
                key={loc}
                onClick={() => {
