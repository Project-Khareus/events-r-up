      ...
      <div className="border-b border-slate-100 sticky top-0 bg-white/80 backdrop-blur-md z-10">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-slate-900">Omnievents Blog</h1>
        ...
        <div className="max-w-7xl mx-auto px-6 overflow-x-auto no-scrollbar">
          <div className="flex items-center gap-8 h-12 text-sm font-medium text-slate-500 whitespace-nowrap">
            {categories.map(cat => (
              <button key={cat} className="hover:text-slate-900 transition-colors">
                {cat}
              </button>
            ))}
          </div>
          ...
          <Link to={createPageUrl("BlogPostDetail") + `?id=${featuredPost.id}`} className="group block">
            <div className="grid lg:grid-cols-2 gap-8 items-center">
              <div className="overflow-hidden rounded-2xl">
                <img 
                  src={featuredPost.cover_image_url || "https://images.unsplash.com/photo-1519167758481-83f550bb49b3?auto=format&fit=crop&q=80&w=2000"} 
